...
public static class CatalogResources
{
    ...
    {
        return (await cache.GetOrCreateAsync("catalog://catalogs", async entry =>
        {
    ...
    public static async Task<string> GetCategoriesAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var cacheKey = $"catalog://categories:{catalogSeoName}:{languageCode}";
    ...
    public static async Task<string> GetProductAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var cacheKey = $"catalog://products:{catalogSeoName}:{productSeoName}:{languageCode}";
        return (await cache.GetOrCreateAsync(cacheKey, async entry =>
        {
            entry.AbsoluteExpirationRelativeToNow = CacheTtl;
