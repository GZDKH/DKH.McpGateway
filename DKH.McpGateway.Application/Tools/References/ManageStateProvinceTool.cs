...
[McpServerToolType]
public static class ManageStateProvinceTool
{
    [McpServerTool(Name = "manage_state_province"), Description(
        "Manage state/provinces: create, update, upsert, delete, get, or list. " +
        "For create/update/upsert: provide state province JSON with fields: code, country, type, " +
        "published, displayOrder, translations [{languageCode, name}]. " +
        "For delete/get: provide state province code. For list: optionally provide search, page, pageSize.")]
    public static async Task<string> ExecuteAsync(
        IApiKeyContext apiKeyContext,
        StateProvinceManagementService.StateProvinceManagementServiceClient client,
        [Description("Action: create, update, upsert, delete, get, or list")] string action,
        [Description("State province JSON (for create/update/upsert)")] string? json = null,
        [Description("State province code (for delete/get)")] string? code = null,
        ...
        IApiKeyContext ctx, string action, string? json, CancellationToken ct)
    {
        ...
        var data = McpProtoHelper.Parser.Parse<StateProvinceData>(json);
        var request = new ManageStateProvinceRequest { Data = data };
        ...
        IApiKeyContext ctx, string? code, CancellationToken ct)
    {
        ...
        var response = await client.DeleteAsync(new DeleteStateProvinceRequest { Code = code }, cancellationToken: ct);
        return McpProtoHelper.FormatManageResponse(response.Success, response.Action, response.Code, response.Errors);
    }
        ...
        IApiKeyContext ctx, string? code, string? language, CancellationToken ct)
    {
        ...
        var response = await client.GetAsync(
            new GetStateProvinceRequest { Code = code, Language = language ?? "" }, cancellationToken: ct);
    ...
    {
        ctx.EnsurePermission(McpPermissions.Read);
        var response = await client.ListAsync(new ListStateProvincesRequest
        {
            Search = search ?? "",
            Page = page ?? 1,
