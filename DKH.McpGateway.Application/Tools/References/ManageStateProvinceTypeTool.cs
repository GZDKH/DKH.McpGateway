...
[McpServerToolType]
public static class ManageStateProvinceTypeTool
{
    [McpServerTool(Name = "manage_state_province_type"), Description(
        "Manage state/province types: create, update, upsert, delete, get, or list. " +
        "For create/update/upsert: provide state province type JSON with fields: code, displayOrder, " +
        "published, translations [{languageCode, name}]. " +
        "For delete/get: provide state province type code. For list: optionally provide search, page, pageSize.")]
    public static async Task<string> ExecuteAsync(
        IApiKeyContext apiKeyContext,
        StateProvinceTypeManagementService.StateProvinceTypeManagementServiceClient client,
        [Description("Action: create, update, upsert, delete, get, or list")] string action,
        [Description("State province type JSON (for create/update/upsert)")] string? json = null,
        [Description("State province type code (for delete/get)")] string? code = null,
        ...
        IApiKeyContext ctx, string? code, CancellationToken ct)
    {
        ...
        var response = await client.DeleteAsync(new DeleteStateProvinceTypeRequest { Code = code }, cancellationToken: ct);
        return McpProtoHelper.FormatManageResponse(response.Success, response.Action, response.Code, response.Errors);
    }
    ...
    {
        ctx.EnsurePermission(McpPermissions.Read);
        var response = await client.ListAsync(new ListStateProvinceTypesRequest
        {
            Search = search ?? "",
            Page = page ?? 1,
