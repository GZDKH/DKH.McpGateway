...
public static class ManageTagsTool
{
    [McpServerTool(Name = "manage_tags"), Description(
        "Manage tags: create, update, delete, get, or list. " +
        "For create/update: provide tag JSON with fields: code, displayOrder, published, " +
    ...
    public static async Task<string> ExecuteAsync(
        ...
        [Description("Tag JSON (for create/update)")] string? json = null,
        ...
        IApiKeyContext ctx, string action, string? json, CancellationToken ct)
    {
        ...
        {
            var request = McpProtoHelper.Parser.Parse<CreateTagRequest>(json);
            var data = await client.CreateAsync(request, cancellationToken: ct);
            return McpProtoHelper.Formatter.Format(data);
        }
        else
        {
            var request = McpProtoHelper.Parser.Parse<UpdateTagRequest>(json);
        ...
        IApiKeyContext ctx, string? code, CancellationToken ct)
    {
        ...
        await client.DeleteAsync(new DeleteTagRequest { Code = code }, cancellationToken: ct);
        return $"Deleted: {code}";
    }
