...
public static class ManageSpecAttributeTool
{
    [McpServerTool(Name = "manage_spec_attribute"), Description(
        "Manage specification attributes: create, update, upsert, delete, get, or list. " +
        "For create/update/upsert: provide spec attribute JSON with fields: code, groupCode, unitCode, displayOrder, " +
    ...
    public static async Task<string> ExecuteAsync(
        ...
        [Description("Spec attribute JSON (for create/update/upsert)")] string? json = null,
        ...
        IApiKeyContext ctx, string action, string? json, CancellationToken ct)
    {
        ...
        var data = McpProtoHelper.Parser.Parse<SpecAttributeData>(json);
        var request = new ManageSpecAttributeRequest { Data = data };
        ...
        IApiKeyContext ctx, string? code, CancellationToken ct)
    {
        ...
        var response = await client.DeleteAsync(new DeleteSpecAttributeRequest { Code = code }, cancellationToken: ct);
        return McpProtoHelper.FormatManageResponse(response.Success, response.Action, response.Code, response.Errors);
    }
