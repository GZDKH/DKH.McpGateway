...
[McpServerToolType]
public static class ManageSpecGroupTool
{
    [McpServerTool(Name = "manage_spec_group"), Description(
        "Manage specification groups: create, update, delete, get, or list. " +
        "For create/update: provide spec group JSON with fields: code, iconName, displayOrder, published, " +
        "isCollapsible, isExpandedByDefault, translations [{languageCode, name, description}], attributes [{...}]. " +
        "For delete/get: provide spec group code. For list: optionally provide search, page, pageSize.")]
    public static async Task<string> ExecuteAsync(
        IApiKeyContext apiKeyContext,
        SpecGroupManagementService.SpecGroupManagementServiceClient client,
        [Description("Action: create, update, delete, get, or list")] string action,
        [Description("Spec group JSON (for create/update)")] string? json = null,
        [Description("Spec group code (for delete/get)")] string? code = null,
        ...
        IApiKeyContext ctx, string action, string? json, CancellationToken ct)
    {
        ...
        {
            var request = McpProtoHelper.Parser.Parse<CreateSpecGroupRequest>(json);
        ...
        {
            var request = McpProtoHelper.Parser.Parse<UpdateSpecGroupRequest>(json);
        ...
        IApiKeyContext ctx, string? code, CancellationToken ct)
    {
        ...
        var response = await client.DeleteAsync(new DeleteSpecGroupRequest { Code = code }, cancellationToken: ct);
        ...
        IApiKeyContext ctx, string? code, string? language, CancellationToken ct)
    {
        ...
        var response = await client.GetAsync(
            new GetSpecGroupRequest { Code = code, Language = language ?? "" }, cancellationToken: ct);
    ...
    {
        ctx.EnsurePermission(McpPermissions.Read);
        var response = await client.ListAsync(new ListSpecGroupsRequest
        {
            Search = search ?? "",
            Page = page ?? 1,
