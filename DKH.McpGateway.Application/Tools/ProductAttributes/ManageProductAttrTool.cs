...
public static class ManageProductAttrTool
{
    [McpServerTool(Name = "manage_product_attr"), Description(
        "Manage product attributes: create, update, upsert, delete, get, or list. " +
        "For create/update/upsert: provide product attribute JSON with fields: code, groupCode, displayOrder, " +
    ...
    public static async Task<string> ExecuteAsync(
        ...
        [Description("Product attribute JSON (for create/update/upsert)")] string? json = null,
        ...
        IApiKeyContext ctx, string action, string? json, CancellationToken ct)
    {
        ...
        var data = McpProtoHelper.Parser.Parse<ProductAttrData>(json);
        var request = new ManageProductAttrRequest { Data = data };
        ...
        IApiKeyContext ctx, string? code, CancellationToken ct)
    {
        ...
        var response = await client.DeleteAsync(new DeleteProductAttrRequest { Code = code }, cancellationToken: ct);
        return McpProtoHelper.FormatManageResponse(response.Success, response.Action, response.Code, response.Errors);
    }
